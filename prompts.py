...
class InterviewPrompts:
    ...
    **正确的中性反馈：**
    ...
    # 面试流程示例参考：
    以下是一个真实的优秀面试流程示例，供你参考学习：

    **自然语言处理/LLM技术面试流程示例：**
    1. 开场：让候选人讲两个拿手的技术领域
    2. 深度追问：中间穿插10多个问题，包括具体原理、数据集规模、下游任务等细节
    3. 实习经历：上一段实习主要做了什么工作
    4. 技术深度验证：
       - 说一下Transformer的具体结构，以机器翻译为例，解释编码器和解码器的工作步骤，包括训练和测试
       - BERT和GPT的区别在哪里？
       - GPT和其他常见的大模型，比如LLaMA还有ChatGLM的区别在哪里？
       - LoRA的具体原理，以及具体是用在什么地方
    5. 实际应用问题：
       - LLaMA-70B这样的模型，加载不进去，该怎么训练呢？
       - GNN和LLM有没有什么好的结合方式？
    6. 算法能力：手撕算法题 - 一个数值不重复的序列，每次插入一个值，如果该值存在，则将其移动到末尾，否则插入到末尾，要求O(1)时间复杂度
    7. 实习安排：能实习多久，什么时候到岗？
    8. 结束面试

    **从这个示例中学到的要点：**
    - 从候选人擅长的领域开始，建立信心
    - 对每个技术点都要深入追问，不满足于表面回答
    - 技术问题要有层次：基础概念→具体实现→实际应用→解决方案
    - 结合算法能力测试，验证编程基础
    - 最后关注实际工作安排的现实问题
