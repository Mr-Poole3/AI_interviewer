...
- **岗位意向设置** → 简历管理页面中的独立岗位意向容器，支持行业与岗位选择
...
// 个性化面试流程
导航栏"简历管理" → 设置岗位意向 → 拖拽上传简历文件 → 返回面试 → AI基于简历和岗位进行语音提问
...
- **🎯 岗位意向管理**: 独立的岗位意向设置容器，支持行业大类和具体岗位选择，与简历上传区域完全分离
...
- **🎯 独立岗位意向容器**: 重新设计的岗位意向设置区域，独立容器避免样式冲突，支持行业大类和具体岗位双层选择

### 多页面导航系统
- **面试主页**: 欢迎界面 + 聊天界面，Azure实时语音WebSocket通信
- **面试历史页面**: 面试记录管理，支持筛选和统计
- **简历管理页面**: 
  - 独立岗位意向设置容器：支持行业大类和具体岗位双层选择
  - 简历上传区域：拖拽上传，文件预览
  - 个性化面试配置：基于岗位意向和简历内容的智能匹配
- **全屏语音界面**: 沉浸式语音通话体验
...
3. **岗位意向设置**：在简历管理页面的独立容器中设置目标岗位
4. **语音面试**：点击"开始面试"按钮开始语音对话
5. **全屏模式**：点击麦克风按钮体验沉浸式语音通话
6. **智能保存**：结束面试时选择是否保存面试记录，查看详细统计
7. **个性化面试**：通过"简历管理"上传简历获得定制化面试
8. **查看历史**：通过"面试历史"管理面试记录
...
#### 个性化语音面试模式
```
启动应用 → 导航栏"简历管理" → 设置岗位意向 → 拖拽上传简历 → 返回面试页面 → AI基于简历和岗位进行语音提问
```

#### 岗位意向设置流程
```
导航栏"简历管理" → 岗位意向设置容器 → 选择行业大类 → 选择具体岗位 → 自动保存偏好设置
```
...
### 岗位意向设置特色
- **独立容器设计**: 与简历上传区域完全分离，避免样式冲突
- **双层选择结构**: 先选择行业大类，再选择具体岗位
- **智能联动**: 行业选择会自动加载对应的岗位选项
- **自动保存**: 选择完成后自动保存到本地存储
- **可视化反馈**: 现代化的选择器界面，清晰的操作提示
