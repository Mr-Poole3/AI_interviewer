...
class HistoryManager {
        ...
        return `
            <div class="history-item" data-id="${interview.id}">
                ...
                <div class="history-content">
                    ...
                    <!-- 详细内容区域（默认折叠） -->
                    <div class="history-details" id="details-${interview.id}" style="display: none;">
                        <div class="details-tabs">
                            <button class="tab-btn active" onclick="historyManager.switchTab('${interview.id}', 'conversation')">
                                <i class="fas fa-comments"></i> 对话记录
                            </button>
                            ${hasEvaluation ? `
                                <button class="tab-btn" onclick="historyManager.switchTab('${interview.id}', 'evaluation')">
                                    <i class="fas fa-chart-bar"></i> 评测报告
                                </button>
                            ` : ''}
                        </div>
                        ...
                        <div class="details-content">
                            <!-- 对话记录标签页 -->
                            <div class="tab-content active" id="conversation-${interview.id}">
                                <div class="conversation-list">
                                    ${this.createConversationHTML(interview.messages || [])}
                                </div>
                            </div>
                            
                            <!-- 评测报告标签页 -->
                            ${hasEvaluation ? `
                                <div class="tab-content" id="evaluation-${interview.id}">
                                    ${this.createEvaluationSummaryHTML(interview.evaluation, interview.id)}
                                </div>
                            ` : ''}
                        </div>
                    </div>
                ...
                <div class="history-actions">
                    <button class="history-action-btn" onclick="historyManager.toggleDetails('${interview.id}')" title="查看详情">
                        <i class="fas fa-eye"></i>
                    </button>
                    ${hasEvaluation ? `
                        <button class="history-action-btn" onclick="historyManager.viewEvaluation('${interview.id}')" title="查看完整评分">
     ...
     * 查看完整评分报告（从摘要页面调用）
     */
    viewFullEvaluation(id) {
        // 如果id为空，尝试从当前显示的详情中获取
        if (!id) {
            const activeDetails = document.querySelector('.history-details[style*="block"]');
            if (activeDetails) {
                id = activeDetails.id.replace('details-', '');
            }
        }
        
        if (id) {
            this.viewEvaluation(id);
        } else {
            alert('无法找到对应的面试记录');
        }
    }
    ...
    toggleDetails(id) {
        ...
        const toggleButton = document.querySelector(`[onclick="historyManager.toggleDetails('${id}')"] i`);
        ...
        if (detailsElement) {
            ...
            // 更新按钮图标
            if (toggleButton) {
                toggleButton.className = isVisible ? 'fas fa-eye' : 'fas fa-eye-slash';
            }
        }
    }
     ...
     * 切换标签页
     */
    switchTab(interviewId, tabType) {
        // 更新标签按钮状态
        const tabButtons = document.querySelectorAll(`#details-${interviewId} .tab-btn`);
        tabButtons.forEach(btn => btn.classList.remove('active'));
        
        const activeButton = document.querySelector(`#details-${interviewId} .tab-btn[onclick*="${tabType}"]`);
        if (activeButton) {
            activeButton.classList.add('active');
        }
        
        // 更新内容显示
        const tabContents = document.querySelectorAll(`#details-${interviewId} .tab-content`);
        tabContents.forEach(content => content.classList.remove('active'));
        
        const activeContent = document.getElementById(`${tabType}-${interviewId}`);
        if (activeContent) {
     ...
     * 创建对话记录HTML
     */
    createConversationHTML(messages) {
        if (!messages || messages.length === 0) {
            return '<div class="no-conversation">暂无对话记录</div>';
        }
        ...
        return messages.map(msg => {
            ...
            const role = msg.type === 'user' ? '候选人' : 'AI面试官';
            const roleClass = msg.type === 'user' ? 'user' : 'assistant';
            const icon = msg.type === 'user' ? 'fa-user' : 'fa-robot';
     ...
     * 创建评测摘要HTML
     */
    createEvaluationSummaryHTML(evaluation, interviewId) {
        if (!evaluation) {
            return '<div class="no-evaluation">暂无评测信息</div>';
        }
            ...
            <div class="evaluation-summary">
                <div class="evaluation-overview">
                    ...
                    <div class="evaluation-text">
                        <h4>评估总结</h4>
                        <p>${evaluation.summary || '暂无评估总结'}</p>
                    </div>
                </div>
                
                <div class="dimension-scores-mini">
                    <h4>各维度评分</h4>
                            ...
                            return `
                                <div class="score-item">
                                    ...
                                    <div class="score-bar-mini">
                                        <div class="score-fill-mini" style="width: ${(score / 10) * 100}%"></div>
                                    </div>
                ...
                <div class="evaluation-highlights">
                    <div class="highlights-section">
                        <h4><i class="fas fa-thumbs-up"></i> 优势表现</h4>
                        <ul>
                            ${(evaluation.strengths || ['表现积极', '回答完整']).map(strength => 
                                `<li>${strength}</li>`
                            ).join('')}
                        </ul>
                    </div>
                    <div class="highlights-section">
                        <h4><i class="fas fa-lightbulb"></i> 改进建议</h4>
                        <ul>
                            ${(evaluation.improvements || ['继续保持', '深入学习']).map(improvement => 
                                `<li>${improvement}</li>`
                            ).join('')}
                        </ul>
                    </div>
                ...
                <div class="evaluation-footer">
                    <button class="btn-primary" onclick="historyManager.viewFullEvaluation('${interviewId || evaluation.interview_id || ''}')">
                        查看完整评分报告
                    </button>
                </div>
            </div>
